...
def SGD(W,X,Y,learning_rate,limit,max_iter):
    cnt = 0
    Jcost_pre = J_function(W,X,Y)
    print ('init cost = ',J_function(W,X,Y))
    x = [-2,7]
    y = [0.0]*2
    while cnt < max_iter:
        for i in range(len(Y)):
        ...
        if (abs(Jcost_pre - J_function(W,X,Y))) < limit:
